...
class TokenAuthentication {
    constructor(http_client, urls, event_bus) {
        ...
        // this.refresh_url_with_prefix = ((urls.refresh_url.prefix != '') ? (urls.refresh_url.prefix + '/') : '') + urls.refresh_url.url;
    ...
    checkToken(is_app_created = false) {
        return new Promise((resolve, reject) => {
            if(!this.access_token) {
                ...
                // this.setAuthHeader(token_obj.access_token);
            }
            else {
                resolve(this.access_token);
            }
    ...
    emitAuthErrorIfNotYet(notify_if_failed = true) {
        if(this.token_refreshing_flag == false) {
            // console.log('send unathorized error');
            // this.emitGlobalEvent('on-unathorized-error');
    ...
    attemptRefreshToken() {
        return new Promise((resolve, reject) => {
            this.http_client.post(this.refresh_url) //`login/refresh` refresh_url_with_prefix
                .then(response => {
...
export class AppAuthentication {
    constructor(http_client, urls, logoutCallback = null, event_bus = null) {//users_roles,
        this.http_client = http_client;
        this.urls = urls;
    ...
    async checkAuthUser() {
        // console.log('entered to check auth');
